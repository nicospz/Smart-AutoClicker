...
data class SavedOverlayButtonSet(
    val id: Long,
    val syncId: String,
    val name: String,
    val priority: Int = 0,
    ...
    companion object {
        ...
        fun fromJson(json: JSONObject): SavedOverlayButtonSet? =
            runCatching {
                SavedOverlayButtonSet(
                    id = json.getLong(KEY_ID),
                    syncId = json.getString(KEY_SYNC_ID),
                    name = json.optString(KEY_NAME).takeIf { it.isNotBlank() } ?: DEFAULT_NAME,
...
private const val DEFAULT_NAME = "Layout"
