...
import com.buzbuz.smartautoclicker.feature.dumb.config.databinding.DialogBaseSelectionBinding
import com.buzbuz.smartautoclicker.feature.dumb.config.di.DumbConfigViewModelsEntryPoint
import com.google.android.material.bottomsheet.BottomSheetDialog
import kotlinx.coroutines.launch
import net.dinglisch.android.tasker.TaskerIntent

class DumbTaskerTaskPickerDialog(
    private val onTaskSelected: (String) -> Unit,
) : OverlayDialog(R.style.AppTheme) {

    private val viewModel: DumbTaskerTaskPickerViewModel by viewModels(
        entryPoint = DumbConfigViewModelsEntryPoint::class.java,
        creator = { dumbTaskerTaskPickerViewModel() },
    )

    private lateinit var viewBinding: DialogBaseSelectionBinding
    private lateinit var adapter: DumbTaskerTaskPickerAdapter

    override fun onCreateView(): ViewGroup {
        viewModel.loadTasks()

        adapter = DumbTaskerTaskPickerAdapter { taskName ->
        ...
        viewBinding = DialogBaseSelectionBinding.inflate(LayoutInflater.from(context)).apply {
            ...
            layoutLoadableList.list.adapter = this@DumbTaskerTaskPickerDialog.adapter
        }
